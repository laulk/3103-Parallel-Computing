...
********************************************************************************************/
        ...
        //2d topology set up
        //initialise dimension and period variables
        ...
        //gives position in topology
        MPI_Cart_coords(new_comm, rank, 2, coords);
        myrow =	coords[0];
        mycol = coords[1];

        //gives ranks of neighbours
...
********************************************************************************************/
		...
		MPI_Sendrecv(&R[1][1],mycolsize, MPI_INT, down, tag,
						 &R[myrowsize+1][1], mycolsize, MPI_INT, up, tag, new_comm, &status);
		MPI_Sendrecv(&G[1][1],mycolsize, MPI_INT, down, tag,
						 &G[myrowsize+1][1], mycolsize, MPI_INT, up, tag, new_comm, &status);
		MPI_Sendrecv(&B[1][1],mycolsize, MPI_INT, down, tag,
						 &B[myrowsize+1][1], mycolsize, MPI_INT, up, tag, new_comm, &status);

		//shifting up
		MPI_Sendrecv(&R[myrowsize][1], mycolsize, MPI_INT, up, tag,
						&R[0][1], mycolsize, MPI_INT, down, tag, new_comm, &status);
		MPI_Sendrecv(&G[myrowsize][1], mycolsize, MPI_INT, up, tag,
						&G[0][1], mycolsize, MPI_INT, down, tag, new_comm, &status);
		MPI_Sendrecv(&B[myrowsize][1], mycolsize, MPI_INT, up, tag,
						&B[0][1], mycolsize, MPI_INT, down, tag, new_comm, &status);

		//shifting right
		int i;
		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = R[i][mycolsize];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, right, tag,
						 recvbuf, myrowsize, MPI_INT, left, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) R[i][0] = recvbuf[i-1];

		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = G[i][mycolsize];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, right, tag,
						 recvbuf, myrowsize, MPI_INT, left, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) G[i][0] = recvbuf[i-1];

		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = B[i][mycolsize];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, right, tag,
						 recvbuf, myrowsize, MPI_INT, left, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) B[i][0] = recvbuf[i-1];

		//shifting left

		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = R[i][1];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, left, tag,
						 recvbuf, myrowsize, MPI_INT, right, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) R[i][mycolsize+1] = recvbuf[i-1];

		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = G[i][1];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, left, tag,
						 recvbuf, myrowsize, MPI_INT, right, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) G[i][mycolsize+1] = recvbuf[i-1];

		for(i=0; i <= myrowsize; i++) sendbuf[i-1] = B[i][1];
		MPI_Sendrecv(sendbuf, myrowsize, MPI_INT, left, tag,
						 recvbuf, myrowsize, MPI_INT, right, tag, new_comm, &status);
		for(i=0; i <= myrowsize; i++) B[i][mycolsize+1] = recvbuf[i-1];
/************************************************************************************************
...
/* Output timing result */
	...
	if(rank==0){
		...
		//Data collection version for print resutls
		//printf("%.6lf \n", t2-t1);  
